...
These comipilations will contain the *means*, not the individual trials.
...
subjective_means <- likert_scores %>% group_by(Interface, question) %>% get_summary_stats(show=c("mean","sd","se")) %>% mutate(ci=1.96*se) %>% ungroup() %>% mutate(question=factor(question, levels=myquestions))
...
ggplot(subjective_means %>% filter(scale=="5pt"), aes(question, mean, color=Interface, fill=Interface))+
  theme_minimal()+
...
ggplot(subjective_means %>% filter(scale=="5pt"), aes(question, mean, color=Interface, fill=Interface))+
  ...
  #theme(strip.text.y = element_text(angle=0))+
  geom_hline(aes(yintercept = 3), linetype=2)+
  labs(title="Subjective Questionnaire Scores")#+facet_grid(question ~ ., scales="free_x")#+facet_grid(scale ~ .)

# ggplot(subjective_means, aes(Interface, mean, color=Interface, fill=Interface))+
#   geom_bar(stat="identity", position="dodge", alpha=myalpha)+
#   geom_errorbar(aes(ymin=mean-ci, ymax=mean+ci), position="dodge", show.legend = FALSE)+
#   scale_color_manual(values=mycolors, guide_legend(title="Interface", reverse = TRUE))+
#   scale_fill_manual(values=mycolors, guide_legend(title="Interface", reverse=TRUE))+
#   coord_flip()+
#   labs(title="Subjective Questionnaire Scores")+
#   facet_grid(vars(question), scales="free", space="free")+#+facet_grid(scale ~ .)
#   theme(strip.text.y = element_text(angle=0))
