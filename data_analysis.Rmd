...
#remove_subjects <- c(12) # insert subject numbers seperated by commas here
...
ggplot(subject_data_all_long, aes(id, totaltime, color=id%in%c(12,30)))+geom_point()+theme(legend.position = "none")+geom_label(data=subject_data_all_long%>%filter(id%in%c(12,30)), aes(label=Interface), nudge_x = 2)+labs(title="Subject Means: Total time", subtite="Subjects 12 and 30 highlighted")
...
sink("anova_data.csv")
# then set p values to be "p < .001" if so and remove the *
...
) %>% adjust_pvalue(p.col="p", output.col="p.adj") #%>% select(Measure=`.y.`, t=statistic, df, `p(Holm)`=p.adj)
