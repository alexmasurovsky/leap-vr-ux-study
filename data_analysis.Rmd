...
# colors
mycolors=c("grey","blue","darkorange") # colorblind safe (I think)
...
geom_label(data=temp_plot_data, aes(Interface, mean, label=round(mean,2)), alpha=1, position=position_nudge(.5), color="white", label.size=.2)+
    ...
    stat_pvalue_manual(data=stat.test %>% filter(p.adj < 0.05), xmin="group1", xmax="group2", label = "p.adj.signif", step.increase=.1, y.position=2*max(temp_plot_data$mean), position=position_nudge(.25))+
    theme(plot.title = element_text(size=title_size), axis.title = element_text(size=axis_text_size), axis.text = element_text(size=axis_text_size*.75))
...
#raincloud releasetime
  releasetime_Interface_raincloud <- ggplot(subject_data_all_long,aes(x=Interface,y=releasetime, fill = Interface, colour = Interface))+
    ...
    stat_pvalue_manual(data=stat.test %>% filter(p.adj < 0.05), xmin="group1", xmax="group2", label = "p.adj.signif", step.increase=.1, y.position=max(subject_data_all_long$releasetime), position=position_nudge(.25))+
    theme(plot.title = element_text(size=title_size), axis.title = element_text(size=axis_text_size), axis.text = element_text(size=axis_text_size*.75))
...
drops_raincloud <-  ggplot(subject_data_all_long, aes(Interface, Drop_Count, fill = Interface, color = Interface))+
   ...
   geom_label(data=temp_plot_data, aes(Interface, y=mean, label=round(mean, 2)), color="white", position=position_nudge(.3), alpha=.9)+ # mean label
    geom_dotplot(binaxis = "y", stackratio=1.4, binwidth = 1, stackdir="down", dotsize=.1, alpha=.8, position=position_nudge(x=0))+
    ...
    geom_errorbar(data = temp_plot_data, aes(x = Interface, y = mean, ymin=mean-(se*1.96), ymax=mean+(se*1.96)), position = position_nudge(.1), colour = "black", width = 0.025, size = 0.9)+ # mean error bar
    ...
    stat_pvalue_manual(data=stat.test %>% filter(p.adj < 0.05), xmin="group1", xmax="group2", label = "p.adj.signif", step.increase=.1, y.position=2.1*max(temp_plot_data$mean), position=position_nudge(.1), alpha = 1)+
  theme(plot.title = element_text(size=title_size), axis.title = element_text(size=axis_text_size), axis.text = element_text(size=axis_text_size*.75))
drops_raincloud
...
# t tests all vs. hhi
# t.tests.likert.all.vs.hhi <- likert_scores %>%
#   ungroup(.) %>% group_by(question) %>%
#   t_test(score ~ Interface, paired=TRUE, ref.group="HHI_Leap") %>%
#   adjust_pvalue() %>% add_significance(p.col="p.adj", output.col="p.adj.signif", symbols=mysymbols) %>%
...
ggplot(subjective_means %>% filter(question=="Agency"), aes(Interface, converted_score, color=Interface, fill=Interface))+
  #theme_minimal()+
  geom_bar(stat="identity", position="dodge", alpha=myalpha, width=.4)+
  ...
  scale_fill_manual(values=mycolors, guide_legend(title="Interface", reverse=TRUE))+
  coord_flip()+
  stat_pvalue_manual(data=t.tests.likert.all.vs.hhi %>% mutate(question=factor(question, levels=myquestions, labels=question_labels), p.adj=paste0("p(Holm) = ", round(p.adj, 3)), y.position=2.9) %>% filter(question == "Agency"), label = "p.adj", step.increase=.1, size=5)+#y.position=statistic
  scale_y_continuous(breaks=c(-3:3), limits=(c(-3,4)))+#labels = c(-2,-1,0,1,2,3))+
  theme(legend.position = "left",  axis.text.y = element_blank(), axis.line.y=element_blank(), axis.ticks.y=element_blank(), strip.text.y = element_text(angle=0))+
  geom_hline(aes(yintercept = 0), linetype=2)+
  labs(title="Subjective Question Scores: Agency", y="Mean Likert-scale Rating", x=NULL)+
  guides(fill = guide_legend(reverse = TRUE), color=guide_legend(reverse=TRUE))#+facet_grid(question ~ .)
ggsave("subjective_questions_new_bar_agency.jpg")
...
ggplot(subjective_means %>% filter(question=="Satisfaction"), aes(Interface, converted_score, color=Interface, fill=Interface))+
  ...
  scale_fill_manual(values=mycolors, guide_legend(title="Interface", reverse=TRUE))+
  coord_flip()+
  stat_pvalue_manual(data=t.tests.likert.all.vs.hhi %>% mutate(question=factor(question, levels=myquestions, labels=question_labels), p.adj=paste0("p(Holm) = ",round(p.adj, 3)), y.position=2.9) %>% filter(question == "Satisfaction"), label = "p.adj", step.increase=.1, size=5)+#y.position=statistic
  scale_y_continuous(breaks=c(-3:3), limits=(c(-3,4)))+#labels = c(-2,-1,0,1,2,3))+
  theme(legend.position = "left",  axis.text.y = element_blank(), axis.line.y=element_blank(), axis.ticks.y=element_blank(), strip.text.y = element_text(angle=0))+
  geom_hline(aes(yintercept = 0), linetype=2)+
  labs(title="Subjective Question Scores: Overall Satisfaction", y="Mean Likert-scale Rating", x=NULL)+
  guides(fill = guide_legend(reverse = TRUE), color=guide_legend(reverse=TRUE))#+facet_grid(question ~ .)
ggsave("subjective_questions_new_bar_satisfaction.jpg")
...
As another analysis, we can use the guidelines offered by Bangor, Kortum and Miller (2009):
<50: Not acceptable  
50–70: Marginal
>70: Acceptable
...
SUS_raincloud <- ggplot(subject_data_all_long, aes(x=Interface, y=SUS, fill=Interface, color=Interface))+
  geom_flat_violin(position = position_nudge(x = .25, y = 0), alpha=myalpha,adjust=mysmoothing)+
  geom_dotplot(binaxis = "y", stackratio=1.4, binwidth = 1, position=position_nudge(x=.2), stackdir="down", dotsize=.6, alpha=.8)+
  ...
  geom_point(data = temp_plot_data, aes(x = Interface, y = mean), position = position_nudge(.25), colour = "BLACK", size=2)+
  geom_label(data = temp_plot_data, aes(x = Interface, y = mean, label=paste0("M = ",round(mean,1))), position = position_nudge(x=.45, y=-2), alpha=1, colour = "white", size=5)+ # mean label
  geom_errorbar(data = temp_plot_data, aes(x = Interface, y = mean, ymin=mean-(se*1.96), ymax=mean+(se*1.96)), position = position_nudge(.25), colour = "BLACK", width = 0.05, size=1)+
  ...
  geom_label(data=SUS_scoring_bins %>% filter(Interface=="B_Leap"), aes(x=.6, y=plot_pos, label=paste0(SUS_bin,":")), color="black", fill="white", alpha=1, size=5)+ # scoring bin name label
  geom_label(data=SUS_scoring_bins, aes(x=Interface, y=plot_pos, label=n), alpha=.6, size=5, color="white")+ #scoring bin label #label=paste0(SUS_bin_percent,"%")),)+
  stat_pvalue_manual(data=stat.test %>% filter(p.adj < 0.05), xmin="group1", xmax="group2", label = "p.adj.signif", step.increase=.1, y.position=1.05*max(subject_data_all_long$SUS), position=position_nudge(.25), size=5)+
  theme(plot.title = element_text(size=title_size*1.25), axis.title = element_text(size=axis_text_size*1.25), axis.text=element_text(size=axis_text_size))
SUS_raincloud
...
all_performance_anovas <- bind_rows(
...
) %>% select(-`p<.05`) #%>% mutate(F=round(F,2))#, p="<.001")
# then set p values to be "p < .001" (if true) and remove the *
all_performance_anovas[which(all_performance_anovas$p<.0001),"p"] <- "p < .0001"
all_performance_anovas
sink("anova_data.csv")
cat("ANOVA's: Performance Measures\n")
write.csv(all_performance_anovas)
sink()

# output table for LaTeX
stargazer(all_performance_anovas, summary=FALSE, rownames = FALSE, title="ANOVA Results: Performance Measures")

```

```{r t-test_results}

Measure=c("Accuracy (Distance)","Total Time","Grab Time","Release Time","Accidental Drops")
